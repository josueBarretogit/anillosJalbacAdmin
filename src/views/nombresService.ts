...
function setNumPages(dataTable: Array<any>): number {
  return Math.ceil((dataTable?.length as number) / totalItems.value) as number;
}

function sliceArray(array: Array<any>, start: number, end: number): Array<any> {
...
export { anillosDataTable, colKey, numPages, page, updatePage };
